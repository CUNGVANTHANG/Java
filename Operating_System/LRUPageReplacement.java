    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < pageReferences.length; i++) {
            ...
            if (!pageFound) {
                ...
                // Tìm trang có thời điểm sử dụng gần đây nhất cao nhất (Least Recently Used)
                int indexToRemove = 0;
                int maxLastUsedTime = lastUsedTime[0];
                for (int j = 1; j < frameSize; j++) {
                    if (lastUsedTime[j] > maxLastUsedTime) {
                        maxLastUsedTime = lastUsedTime[j];
                        indexToRemove = j;
                    }
                }

                // Thay thế trang có thời điểm sử dụng gần đây nhất cao nhất bằng trang mới
                frameArray[indexToRemove] = page;
                lastUsedTime[indexToRemove] = 0;
